"""TikTok channel analyzer."""

from __future__ import annotations

from datetime import datetime
from statistics import mean
from typing import Any

from contentos_growth.application.channel_analysis_types import ChannelAnalysisResult
from contentos_growth.application.platform_analyzers.base import (
    avg_metric,
    build_recommendations,
    clamp_score,
    detect_cta_patterns,
    extract_hashtags,
    posting_frequency_days,
)


def analyze_tiktok_channel(
    *,
    channel_id: str,
    project_id: str,
    channel_name: str,
    overview: dict[str, Any],
) -> ChannelAnalysisResult:
    totals = overview.get("channel_totals") or {}
    media_items = overview.get("media_items") or []

    display_name = str(totals.get("display_name") or channel_name)
    followers = int(totals.get("follower_count") or 0)
    video_count = int(totals.get("video_count") or len(media_items))
    likes_total = int(totals.get("likes_count") or 0)

    titles = [str((item.get("metrics") or {}).get("title") or item.get("title") or "") for item in media_items]
    hashtags = extract_hashtags(*titles)
    cta_patterns = detect_cta_patterns(*titles)
    avg_engagement = avg_metric(media_items, "engagement_rate")
    avg_views = avg_metric(media_items, "views")
    avg_shares = avg_metric(media_items, "shares")
    posting_gap_days = posting_frequency_days(media_items)

    branding_score = 40.0
    if display_name:
        branding_score += 20
    if followers >= 500:
        branding_score += 20
    if likes_total >= 1000:
        branding_score += 10

    consistency_score = 35.0
    if posting_gap_days is not None:
        if posting_gap_days <= 1:
            consistency_score += 40
        elif posting_gap_days <= 3:
            consistency_score += 30
        elif posting_gap_days <= 7:
            consistency_score += 15
    if video_count >= 10:
        consistency_score += 10

    format_score = 45.0
    if video_count >= 3:
        format_score += 25
    if avg_views and avg_views >= 1000:
        format_score += 20

    engagement_score = 30.0
    if avg_engagement is not None:
        engagement_score += min(avg_engagement * 700, 50)
    if avg_shares and avg_views and avg_views > 0:
        share_rate = avg_shares / avg_views
        engagement_score += min(share_rate * 2000, 15)

    metadata_score = 25.0
    if len(hashtags) >= 3:
        metadata_score += 30
    elif hashtags:
        metadata_score += 10
    if cta_patterns:
        metadata_score += 20
    if titles:
        metadata_score += 10

    dimensions = {
        "branding": clamp_score(branding_score),
        "consistency": clamp_score(consistency_score),
        "format_mix": clamp_score(format_score),
        "engagement": clamp_score(engagement_score),
        "metadata": clamp_score(metadata_score),
    }
    growth_score = clamp_score(mean(dimensions.values()))
    tone = "viral" if avg_engagement and avg_engagement >= 0.08 else "descoberta"

    report = {
        "channel_name": display_name,
        "content_style": {
            "avg_views": avg_views,
            "avg_shares": avg_shares,
            "video_count": video_count,
            "tone": tone,
        },
        "frequency": {"avg_days_between_posts": posting_gap_days},
        "hashtags": hashtags,
        "cta_patterns": cta_patterns,
        "audience": {"likely_profile": "audiência TikTok", "follower_count": followers},
        "dimensions": dimensions,
        "data_source": {
            "snapshot_id": overview.get("id"),
            "fetched_at": overview.get("fetched_at"),
            "media_sample_size": len(media_items),
            "platform": "tiktok",
        },
    }

    profile = {
        "audience": "audiência TikTok",
        "tone": tone,
        "posting_gap_days": posting_gap_days,
        "follower_count": followers,
        "hashtags": hashtags[:10],
        "cta_patterns": cta_patterns,
    }

    recommendations = build_recommendations(
        project_id=project_id,
        channel_id=channel_id,
        dimensions=dimensions,
        hashtags=hashtags,
        cta_patterns=cta_patterns,
        posting_gap_days=posting_gap_days,
        format_hint="vídeos curtos",
    )

    summary = (
        f"TikTok {display_name} analisado com score {growth_score:.0f}/100. "
        f"{followers:,} seguidores, {len(media_items)} vídeos amostrados, "
        f"{len(recommendations)} recomendação(ões)."
    ).replace(",", ".")

    return ChannelAnalysisResult(
        channel_id=channel_id,
        project_id=project_id,
        platform="tiktok",
        channel_name=channel_name,
        score=growth_score,
        summary=summary,
        report=report,
        profile=profile,
        recommendations=recommendations,
        analyzed_at=datetime.utcnow().isoformat() + "Z",
    )
